"""Drawing of every edge opening in the case, from the same numbers as the scad."""
import cairosvg
...
# name, edge, x0, x1, z0, z1
PORTS = [
    ("Audio, 3.5 mm TRRS", "top",    -44.35, -36.25, -7.40, -0.50),
    ("USB-C",              "top",     22.20,  32.80, -5.70,  0.40),
    ("microSD",            "bottom",  14.90,  28.20, -4.30, -0.20),
    ("Power, SW4",         "bottom", -34.50, -24.00, -4.20, -0.50),
]

S  = 1.25                       # drawing scale
...
g.append(txt(18, 15, "Fri3d badge 2026 - every opening in the case", 6.2, w=700, fill="#1c1c1a"))
g.append(txt(18, 22, "Four of them, all in the long edges. The short ends are closed, and so are the "
                     "back and the front. The LoRa antenna is left closed.", 3.4, fill="#6b6b64"))
...
g.append(txt(sx(-62.2), sy(0)+1.2, "plan", 3.2, fill="#a8a49a"))

for naam, rand, x0, x1, z0, z1 in PORTS:
    yb = 29.7 if rand == "top" else -29.7
    yy = sy(yb)
    g.append(f'<rect x="{sx(x0):.2f}" y="{yy-1.5:.2f}" width="{(x1-x0)*S:.2f}" height="3.0" fill="#c9312a"/>')
    lx = sx((x0+x1)/2)
    ly = yy + (-7 if rand == "top" else 7)
    g.append(f'<path d="M {lx:.2f},{yy:.2f} L {lx:.2f},{ly:.2f}" stroke="#c9312a" stroke-width="0.3"/>')
    g.append(txt(lx, ly + (-1.6 if rand == "top" else 4.2), naam, 3.4, "middle", w=700, fill="#c9312a"))

# ---- elevations of the two long edges
for rand, base, kop in [("top",    157.0, "top edge, seen from outside"),
                        ("bottom", 200.0, "bottom edge, seen from outside")]:
    top = ez(DECK_Z1, base)
    ...
    g.append(txt(sx(-62.2)+2.0, ez(0.9,base)-1.3, "seam, front plate / back shell", 2.6, fill="#a8a49a"))
...
g.append(txt(18, PH-6, "Generated from fri3d_badge_2026_case.scad, module edge_ports(). "
            "Measured from the centre of the PCB, z = 0 on the top face of the board. "
            "Exact coordinates are in README.md.",
            2.8, fill="#a8a49a"))
g.append('</svg>')
open("openings.svg","w").write("\n".join(g))
cairosvg.svg2pdf(url="openings.svg", write_to="openings.pdf")
cairosvg.svg2png(url="openings.svg", write_to="openings.png", scale=4)
print("ok")
